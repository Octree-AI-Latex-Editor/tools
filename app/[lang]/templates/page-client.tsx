...
import {
  ...
  getLocalizedTemplateCategories,
...
export default function TemplatesPageClient({ dict, lang }: TemplatesPageClientProps) {
  const [searchQuery, setSearchQuery] = useState("");
  const allTemplatesLabel = dict.templates.allTemplates;
  const [selectedCategory, setSelectedCategory] =
    useState<CategoryFilter>(allTemplatesLabel as CategoryFilter);
  ...
  const filteredTemplates = useMemo(() => {
    return localizedTemplates.filter((template) => {
      ...
      const matchesCategory =
        selectedCategory === allTemplatesLabel ||
        template.category === selectedCategory;
      return matchesSearch && matchesCategory;
    });
  }, [searchQuery, selectedCategory, allTemplatesLabel]);
  ...
  const categoryCounts = useMemo(() => {
    const counts: Record<string, number> = {
      [allTemplatesLabel]: localizedTemplates.length,
    };
    localizedTemplates.forEach((template) => {
      counts[template.category] = (counts[template.category] || 0) + 1;
    });
    return counts;
  }, [allTemplatesLabel, localizedTemplates]);
  ...
  const structuredData = useMemo(() => ({
    ...
    mainEntity: {
      "@type": "ItemList",
      itemListElement: localizedTemplates.map((template, index) => ({
        "@type": "ListItem",
        position: index + 1,
        item: {
          ...
          <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div className="flex items-center justify-center gap-2 mb-6">
              <Link
                ...
                title={dict.common.starOnGitHub}
              >
              ...
              <Link
                ...
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded-lg transition-colors"
                title={dict.common.joinUsOnReddit}
              >
              ...
              <Link
                ...
                rel="noopener noreferrer"
                className="inline-flex items-center justify-center p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-200 rounded-lg transition-colors"
                title={dict.common.joinOurDiscord}
              >
              ...
              <Tabs defaultValue="templates" className="w-auto">
                <TabsList>
                  <TabsTrigger value="tools" asChild>
                    <Link href={`/${lang}`}>{dict.common.tools}</Link>
                  </TabsTrigger>
                  <TabsTrigger value="templates" asChild>
                    <Link href={`/${lang}/templates`}>{dict.common.templates}</Link>
                  </TabsTrigger>
                </TabsList>
              </Tabs>
            </div>
            ...
            <div className="max-w-xl mx-auto">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder={dict.common.searchTemplates}
                />
              </div>
            </div>
          </div>
                ...
                <nav className="space-y-1">
                  {localizedCategories.map((cat) => {
                    ...
                    const getCategoryLabel = (name: string) => {
                      if (name === allTemplatesLabel) return allTemplatesLabel;
                      const categories = dict.categories as Record<string, string>;
                      const categoryMap: Record<string, string> = {
                        'Academic': categories.academic,
                        'Presentations': categories.presentations,
                        'Resume & CV': categories.resumeCv,
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-sm text-gray-500">
                  {dict.common.templatesCount
                    .replace('{count}', filteredTemplates.length.toString())
                    .replace('{plural}', filteredTemplates.length !== 1 ? 's' : '')}
                </span>
              </div>
              ...
              {filteredTemplates.length === 0 && (
                <div className="text-center py-12">
                  <p className="text-gray-500">
                    {dict.templates.noTemplatesFound}
                  </p>
                </div>
              )}
